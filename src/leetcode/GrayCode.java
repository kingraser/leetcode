...
public class GrayCode {
    ...
    */
    ...
    public void test() {
        ...
        Assert.assertEquals(Arrays.asList(0, 1, 3, 2), grayCodeII(2));
    }
    ...
    public List<Integer> grayCodeII(int n) {
        List<Integer> result = new ArrayList<>(1 << n);// 2^n
        result.add(0);
        for (int highestBit = 1; n-- > 0; highestBit <<= 1)
            for (int j = highestBit; --j >= 0; result.add(highestBit | result.get(j))); // 要反着遍历,才能对称
        return result;
    }
}
