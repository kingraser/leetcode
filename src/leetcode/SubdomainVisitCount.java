...
public class SubdomainVisitCount {
    ...
    */
    ...
    public List<String> subdomainVisits(String[] domains) {
        ...
        for (String domain : domains) count(domain, map);
    ...
    private void count(String s, Map<String, Integer> map) {
        int spaceIdx = s.indexOf(' '), count = Integer.parseInt(s.substring(0, spaceIdx));
        map.merge(s.substring(spaceIdx + 1), count, Integer::sum);
        for (int dotIdx = spaceIdx + 2; dotIdx > 0; )
            if ((dotIdx = s.indexOf('.', dotIdx)) > 0) map.merge(s.substring(++dotIdx), count, Integer::sum);
    }
