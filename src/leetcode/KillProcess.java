...
public class KillProcess {
  ...
  */
  ...
  @Test
  public void test() {
    assertEquals(Arrays.asList(5, 10), killProcess(Arrays.asList(1, 3, 10, 5), Arrays.asList(3, 0, 5, 3), 5));
  }

  public List<Integer> killProcess(List<Integer> pid, List<Integer> ppid, int kill) {
    Map<Integer, List<Integer>> map = new HashMap<>();
    IntStream.range(0, ppid.size())
        .forEach(i -> map.computeIfAbsent(ppid.get(i), k -> new ArrayList<>()).add(pid.get(i)));
    LinkedList<Integer> result = new LinkedList<>(), stack = new LinkedList<>(Arrays.asList(kill));
    for (int cur; !stack.isEmpty(); result.add(cur = stack.pop()), stack
        .addAll(map.getOrDefault(cur, new ArrayList<>())));
    return result;
  }
}
