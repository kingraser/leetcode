    ...
    */
    ...
    public void test() {
        TestUtil.testEquals(new Object[][]{
                {new int[]{102, 120, 130, 132, 210, 230, 302, 310, 312, 320}, new int[]{2, 1, 3, 0}},
                {new int[]{222, 228, 282, 288, 822, 828, 882}, new int[]{2, 2, 8, 8, 2}},
                {new int[]{}, new int[]{3, 7, 5}},
                {new int[]{200}, new int[]{0, 2, 0, 0}},
                {new int[]{}, new int[]{0, 0, 0}}
        });
    }

    public int[] findEvenNumbers(int[] digits) {
        List<Integer> result = new ArrayList<>(9 * 10 * 5);
        int digitMap[] = new int[10];
        for (int digit : digits) digitMap[digit]++;
        dfs(result, digitMap, 0);
        return result.stream().mapToInt(Integer::intValue).toArray();
    }

    void dfs(List<Integer> result, int[] digitMap, int val) {
        if (val > 99) {
            result.add(val);
            return;
        }
        val *= 10;
        for (int i = 0; i < 10; i++) {
            if (digitMap[i] == 0 || (val == 0 && i == 0) || (val > 99 && (i & 1) == 1)) continue;
            digitMap[i]--;
            dfs(result, digitMap, val += i);
            val -= i;
            digitMap[i]++;
        }
    }
