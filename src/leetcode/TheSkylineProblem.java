...
public class TheSkylineProblem {
	...
	*/
	...
	public void test() {
		...
		assertArrayEquals(expected.toArray(), getSkyline(input).toArray());
	}

	public List<int[]> getSkyline(int[][] buildings) {
		List<int[]> res = new ArrayList<>();
		PriorityQueue<Node> nodes = new PriorityQueue<>((n1, n2) -> n1.x != n2.x
				? n1.x - n2.x :
				n1.isLeft && n2.isLeft
						? n2.y - n1.y :
						!n1.isLeft && !n2.isLeft ? n1.y - n2.y
								: n1.isLeft ? -1 : 1);
		...
		for (Node node; !nodes.isEmpty(); ) {
			...
			if (!Objects.equals(pre, heap.peek())) res.add(new int[]{node.x, heap.peek()});
		}
		return res;
	}
