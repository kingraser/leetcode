	...
	*/
	...
	public void test() {
		TestUtil.testEquals(new Object[][]{
				{List.of("e", "b"), new String[]{"e", "a", "b"}, new int[]{0, 0, 1}},
				{List.of("a", "b", "c"), new String[]{"a", "b", "c", "d"}, new int[]{1, 0, 1, 1}},
		});
	}

	public List<String> getLongestSubsequence(String[] words, int[] groups) {
		List<String> result = new ArrayList<>(groups.length) {{add(words[0]);}};
		for (int i = 0, last = groups.length - 1; i < last; )
			if (groups[i++] != groups[i]) result.add(words[i]);
		return result;
	}
}
