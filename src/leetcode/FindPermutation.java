...
public class FindPermutation {
  ...
  */
  ...
  public void test() {
    ...
    assertArrayEquals(new int[] { 2, 1, 3 }, findPermutation("DI"));
  }

  public int[] findPermutation(String s) {
    int result[] = IntStream.range(1, s.length() + 2).toArray();
    for (int idx = 0, start; idx < s.length(); idx++)
      if (s.charAt(idx) == 'D') {
        for (start = idx; idx < s.length() && s.charAt(idx) == 'D'; idx++);
        ArrayUtil.reverse(result, start, idx);
      }
    return result;
  }
}
