...
public class TeemoAttacking {
  ...
  */
  ...
  public int findPoisonedDuration(int[] timeSeries, int duration) {
    if (timeSeries.length < 2) return timeSeries.length * duration;
    int result = 0, left = timeSeries[0], right = left + duration;
    for (int i = 1; i < timeSeries.length; right = timeSeries[i++] + duration)
