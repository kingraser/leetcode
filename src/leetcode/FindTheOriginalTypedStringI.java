package leetcode;

import leetcode.util.TestUtil;
import org.junit.Test;

public class FindTheOriginalTypedStringI {
    /*
    Alice is attempting to type a specific string on her computer.
    However, she tends to be clumsy and may press a key for too long, resulting in a character being typed multiple times.
    Although Alice tried to focus on her typing, she is aware that she may still have done this at most once.
    You are given a string word, which represents the final output displayed on Alice's screen.
    Return the total number of possible original strings that Alice might have intended to type.

    Example 1:
    Input: word = "abbcccc"
    Output: 5
    Explanation:
    The possible strings are: "abbcccc", "abbccc", "abbcc", "abbc", and "abcccc".

    Example 2:
    Input: word = "abcd"
    Output: 1
    Explanation:
    The only possible string is "abcd".

    Example 3:
    Input: word = "aaaa"
    Output: 4

    Constraints:
        1 <= word.length <= 100
        word consists only of lowercase English letters.
    */
    @Test
    public void test() {
        TestUtil.testEquals(new Object[][]{
                {2, "aa"},
                {5, "abbcccc"},
                {1, "abcd"},
                {4, "aaaa"},
        });
    }

    public int possibleStringCountII(String word) {
        int length = word.length(), result = length;
        for (int i = 1, prev = word.charAt(0); i < length; ) if (prev != (prev = word.charAt(i++))) result--;
        return result;
    }

    public int possibleStringCount(String word) {
        int result = 1;
        for (int length = word.length(), index = 0, startChar = word.charAt(0), start, currentChar = 0; index < length; result += index - start, startChar = currentChar)
            for (start = ++index; index < length && (currentChar = word.charAt(index)) == startChar; ) index++;
        return result;
    }
}
