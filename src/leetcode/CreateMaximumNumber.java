...
public class CreateMaximumNumber {
    ...
    */
    ...
    public void test() {
        ...
        Assert.assertArrayEquals(new int[] { 5, 5, 4 }, maxNumber(new int[] { 5, 5, 1 }, new int[] { 4, 0, 1 }, 3));
        Assert.assertArrayEquals(
                new int[] { 9, 9, 9, 9, 9, 8, 7, 5, 6, 3, 4, 2, 4, 7, 4, 5, 7, 7, 2, 5, 6, 3, 6, 7, 2, 2, 8, 4, 6, 0, 4,
                        7, 8, 9, 1, 7, 0, 3, 5, 3, 2, 8, 1, 6, 6, 1, 0, 8, 4, 0 },
                maxNumber(
                        new int[] { 8, 0, 4, 4, 1, 7, 3, 6, 5, 9, 3, 6, 6, 0, 2, 5, 1, 7, 7, 7, 8, 7, 1, 4, 4, 5, 4, 8,
                                7, 6, 2, 2, 9, 4, 7, 5, 6, 2, 2, 8, 4, 6, 0, 4, 7, 8, 9, 1, 7, 0 },
                        new int[] { 6, 9, 8, 1, 1, 5, 7, 3, 1, 3, 3, 4, 9, 2, 8, 0, 6, 9, 3, 3, 7, 8, 3, 4, 2, 4, 7, 4,
                                5, 7, 7, 2, 5, 6, 3, 6, 7, 0, 3, 5, 3, 2, 8, 1, 6, 6, 1, 0, 8, 4 },
                        50));
    }

    public int[] maxNumber(int[] A1, int[] A2, int k) {
        int[] result = new int[k], tmp = new int[k];
        List<int[]> dp1 = dp(A1), dp2 = dp(A2);
    ...
    private List<int[]> dp(int[] A) {
        List<int[]> res = Lists.newArrayList(A);
        for (int idx = 0; res.size() < A.length;) {
            int[] B = res.get(res.size() - 1), C = new int[B.length - 1];
            if (idx > 0 && B[idx - 1] < B[idx]) idx--;
            else for (; idx < C.length && B[idx] >= B[idx + 1]; idx++);
