    ...
    */
    ...
    public void test() {
        TestUtil.testEquals(
                new Object[]{1, 1, -1},
                new Object[][]{
                        {new int[][]{{0, 0, 8}, {2, 2, 9}}, new int[]{3, 4}},
                        {new int[][]{{2, 1, 5}, {4, 4, 5}, {6, 6, 8}}, new int[]{5, 5}},
                        {new int[][]{{4, 4, 5}}, new int[]{8, 6}},
                }
        );
    }

    public int nearestDrone(int[][] drones, int[] target) {
        int currentMinDistance = Integer.MAX_VALUE, i = 0, result = -1;
        for (int[] drone : drones) {
            int distance = calcDistance(drone, target);
            if (distance <= drone[2] && distance < currentMinDistance) {
