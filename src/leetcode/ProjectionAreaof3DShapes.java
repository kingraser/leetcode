...
public class ProjectionAreaof3DShapes {
    ...
    */
    ...
    public int projectionArea(int[][] grid) {
        int result = 0;
        for (int i = 0, length = grid.length, bestRow, bestCol, j; i < grid.length; result += bestRow + bestCol, i++)
            for (j = 0, bestRow = 0, bestCol = 0; j < length; bestRow = Math.max(bestRow, grid[i][j]), bestCol = Math.max(bestCol, grid[j][i]), j++)
