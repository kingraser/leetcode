...
public class NumberofLinesToWriteString {
    ...
    */
    ...
    public int[] numberOfLines(int[] widths, String s) {
        int lineCount = 1, lastLineWidth = 0, lineWidth = 100;
        for (int i = 0, currentWidth; i < s.length(); i++)
            if ((lastLineWidth += currentWidth = widths[s.charAt(i) - 'a']) > lineWidth) {
                lineCount++;
                lastLineWidth = currentWidth;
            }
        return new int[]{lineCount, lastLineWidth};
    }
}
