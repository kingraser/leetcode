...
public class TestUtil {
	...
	public static String toString(Object o) {
		if (Objects.isNull(o) || !o.getClass().isArray()) {
	...
	private static void assertArrayEquals(Object expected, Object actual) {
		...
		} else if (Objects.equals(expected.getClass().getComponentType(), boolean.class)) {
			Assert.assertArrayEquals((boolean[]) expected, (boolean[]) actual);
		} else {
			throw new UnsupportedOperationException("primitive types only support int and double and long and boolean right now!");
		}
	}
	...
	public static Object getArray(String s) {
		...
		Stack<TestArray> stack = new Stack<>() {{
			push(root);
		}};
	...
	public static List<List<String>> transferToString(String s) {
		return Arrays.stream(TestUtil.getInputs(s))
				.map(Arrays::stream)
				.map(stream -> stream.map(e -> (String) e).collect(Collectors.toList()))
				.collect(Collectors.toList());
	}

	public static List<List<Integer>> transferToInteger(String s) {
		return Arrays.stream(TestUtil.getInputs(s))
				.map(Arrays::stream)
				.map(stream -> stream.map(e -> (Integer) e).collect(Collectors.toList()))
				.collect(Collectors.toList());
	}
