...
public class Dota2Senate {
  ...
  */
  ...
  public String predictPartyVictory(String senate) {
    Deque<Integer> r = new ArrayDeque<Integer>(), d = new ArrayDeque<Integer>();
    for (int i = 0, length = senate.length(); i < length; i++)
      if (senate.charAt(i) == 'R') r.add(i);
      else d.add(i);
    for (int rIdx, dIdx, length = senate.length(); !r.isEmpty() && !d.isEmpty();)
      if ((rIdx = r.poll()) < (dIdx = d.poll())) r.add(rIdx + length);
      else d.add(dIdx + length);
