...
public class CheckASCIIPalindromic {
    ...
    */
    ...
    public boolean isPalindromic(String s) {
        for (int left = 0, right = s.length() - 1, l; left <= right; ) {
            if ((l = s.charAt(left++)) == 'n') {
                if (s.charAt(right--) != 'v') return false;
            } else if (l == 'v') {
                if (s.charAt(right--) != 'n') return false;
            } else if (l == 'f') {
                if (s.charAt(right--) != 'f') return false;
            } else return false;
        }
        return true;
    }
