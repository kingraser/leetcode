...
public class CoinPath {
  ...
  */
    ...
    public List<Integer> cheapestJump(int[] A, int B) {
        int len = A.length, cost[] = IntStream.range(0, len).map(j -> Integer.MAX_VALUE).toArray(),
                previousIdx[] = IntStream.range(0, len).map(j -> -1).toArray(), length[] = new int[len], i;
