    ...
    */
    ...
    public void test() {
        TestUtil.testEquals(new Object[][]{
                {2.65000D, TestUtil.getArray("[[3,50],[7,10],[12,25]]"), 10},
                {0.25000D, TestUtil.getArray("[[1,0],[4,25],[5,50]]"), 2},
                {0.00000D, TestUtil.getArray("[[2,50]]"), 0},
        });
    }
    ...
    public double calculateTax(int[][] brackets, int income) {
        ...
        for (int[] bracket : brackets) {
            int money = Integer.min(bracket[0], income) - last;
            result += ((double) money * bracket[1]) / 100;
            if ((last = bracket[0]) >= income) break;
        }
        return result;
    }
}
